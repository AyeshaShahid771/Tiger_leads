...
class AdminUser(Base):
    ...
    role = Column(String(50), nullable=True)  # admin, ops, billing
...
class PendingUserType(Base):
    """
    Stores user type (trade/category) requests that require admin approval.
    """

    __tablename__ = "pending_user_types"

    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(
        Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )
    user_role = Column(String(50), nullable=False)  # 'Contractor' or 'Supplier'
    user_type_value = Column(String(255), nullable=False)  # The actual trade name
    status = Column(String(20), default="pending")  # 'pending', 'approved', 'rejected'
    created_at = Column(DateTime, server_default=func.now())
    reviewed_at = Column(DateTime, nullable=True)
    reviewed_by = Column(
