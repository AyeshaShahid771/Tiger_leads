...
def get_registration_status(
    ...
    Requires authentication token in header.
    """
    logger.info(f"Supplier registration status request from user: {current_user.email}")

    if current_user.role != "Supplier":
        return {
            "has_supplier_role": False,
            "message": "Please set your role to 'Supplier' first",
        }
    ...
    if supplier.registration_step < 4:
        next_step = supplier.registration_step + 1
    ...
    return {
        ...
        "message": (
            ...
            else f"Please complete Step {next_step}"
        ),
    }
