...
def migrate_to_country_city():
    ...
    try:
        ...
        try:
            ...
            result = session.execute(text("SELECT COUNT(*) FROM jobs WHERE country_city IS NOT NULL"))
            count = result.scalar()
        ...
        response = input("Do you want to DROP the old 'city' and 'country' columns? (yes/no): ").strip().lower()
        ...
        if response == 'yes':
            ...
            print("⚠ Keeping old city and country columns (you can drop them manually later)")
