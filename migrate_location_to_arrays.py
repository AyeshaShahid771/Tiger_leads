    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            if current_type == 'ARRAY':
                ...
                conn.execute(text("ALTER TABLE contractors ADD COLUMN IF NOT EXISTS state_temp TEXT[]"))
                conn.execute(text("ALTER TABLE contractors ADD COLUMN IF NOT EXISTS country_city_temp TEXT[]"))
                ...
                contractors = conn.execute(text("SELECT id, state, country_city FROM contractors")).fetchall()
                ...
                for contractor_id, state_val, city_val in contractors:
                    ...
                    if state_val:
                        try:
                            ...
                            state_array = parsed if isinstance(parsed, list) else [str(parsed)]
                        except:
                    ...
                    if city_val:
                        try:
                            ...
                            city_array = parsed if isinstance(parsed, list) else [str(parsed)]
                        except:
                            city_array = [city_val]
                    
                    if state_array:
                        conn.execute(text("UPDATE contractors SET state_temp = :val WHERE id = :id"), 
                                   {"val": state_array, "id": contractor_id})
                    if city_array:
                        conn.execute(text("UPDATE contractors SET country_city_temp = :val WHERE id = :id"), 
                ...
                conn.execute(text("ALTER TABLE contractors RENAME COLUMN state_temp TO state"))
                conn.execute(text("ALTER TABLE contractors RENAME COLUMN country_city_temp TO country_city"))
            ...
            if current_type == 'ARRAY':
                ...
                print("   - Adding temporary array columns...")
                conn.execute(text("ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS service_states_temp TEXT[]"))
                conn.execute(text("ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS country_city_temp TEXT[]"))
                
                # Get all suppliers to convert JSON strings to arrays
                print("   - Converting JSON strings to arrays...")
                result = conn.execute(text("SELECT id, service_states, country_city FROM suppliers"))
                ...
                for supplier in suppliers:
                    ...
                    if country_city_str:
                        try:
                            ...
                            country_city_array = parsed if isinstance(parsed, list) else [str(parsed)]
                        except:
                    ...
                    if service_states_array:
                        conn.execute(
                            text("UPDATE suppliers SET service_states_temp = :states WHERE id = :id"),
                            {"states": service_states_array, "id": supplier_id}
                        )
                    
                    if country_city_array:
                        conn.execute(
                            text("UPDATE suppliers SET country_city_temp = :cities WHERE id = :id"),
                ...
                conn.execute(text("ALTER TABLE suppliers RENAME COLUMN service_states_temp TO service_states"))
                conn.execute(text("ALTER TABLE suppliers RENAME COLUMN country_city_temp TO country_city"))
