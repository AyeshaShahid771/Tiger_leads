...
def migrate():
    ...
    with engine.connect() as conn:
        try:
            ...
            try:
                conn.execute(text("ALTER TABLE contractors DROP COLUMN IF EXISTS service_zip_code"))
                conn.execute(text("ALTER TABLE contractors DROP COLUMN IF EXISTS state"))
                conn.execute(text("ALTER TABLE contractors DROP COLUMN IF EXISTS service_state"))
            ...
            print("\nSummary:")
            print("- Contractors: now have single 'country_city' field (format: USA/New York)")
            print("- Suppliers: now have single 'country_city' field (format: USA/Miami)")
