...
def get_profile_info(
    ...
    if main_user.role == "Contractor":
        ...
        if contractor:
            ...
            response["user_type"] = contractor.user_type
        else:
            response["state"] = None
    ...
    # Get supplier location data
        ...
        if supplier:
            ...
            response["user_type"] = supplier.user_type
        else:
            response["service_states"] = None
            response["country_city"] = None
            response["user_type"] = None

    # Get pending and rejected user types for this user
    pending_user_types = (
        db.query(models.user.PendingUserType)
        .filter(
            models.user.PendingUserType.user_id == effective_user_id,
    ...
    response["pending_user_types"] = pending_list if pending_list else None
