...
class GroqEmailRequest(BaseModel):
    ...
    permit: Optional[str] = Field(None, max_length=100)  # Ignored, kept for backward compatibility
    job_id: Optional[int] = None
    permit_type_norm: Optional[str] = Field(None, max_length=100)
...
class GroqService:
    ...
    def _build_prompt(self, data: dict) -> str:
        ...
        # Strictly use normalized type. If it's missing or looks like a permit number (contains 3+ digits), default to 'Construction'
        permit_type = data.get('permit_type_norm') or 'Construction'
        if re.search(r'\d{3,}', permit_type):
...
❌ NO permit numbers anywhere in the email
...
✗ Permit numbers anywhere in the email
...
def generate_email_template(
    ...
    # Fetch job data from DB if job_id is provided to ensure we have the correct normalized type
    if payload.job_id:
        try:
            job = db.query(models.user.Job).filter(models.user.Job.id == payload.job_id).first()
            if job:
                if job.permit_type_norm:
                    payload_data["permit_type_norm"] = job.permit_type_norm
                if job.project_description:
                    payload_data["job_description"] = job.project_description
        except Exception as e:
            logger.error(f"Failed to fetch job data for job_id {payload.job_id}: {e}")
