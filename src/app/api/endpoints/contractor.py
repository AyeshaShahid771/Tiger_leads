...
def contractor_step_2(
    ...
    try:
        ...
        if data.state:
            ...
            existing_states = contractor.state or []
            combined_states = existing_states + incoming_states
            unique_states = list(dict.fromkeys(combined_states))
            duplicates_removed += len(combined_states) - len(unique_states)
            contractor.state = unique_states
        ...
        if data.country_city:
            ...
            existing_cities = contractor.country_city or []
            combined_cities = existing_cities + incoming_cities
            unique_cities = list(dict.fromkeys(combined_cities))
            duplicates_removed += len(combined_cities) - len(unique_cities)
            contractor.country_city = unique_cities
        ...
        base_message = "Service jurisdictions saved successfully"
...
async def contractor_step_3(
    ...
    try:
        ...
        # Save user_type - merge with existing, preserving order
        if data.user_type:
            incoming_types = (
        ...
        base_message = "Trade information saved successfully"
        if duplicates_removed > 0:
            base_message += f". {duplicates_removed} duplicate trades were ignored."
...
@router.get("/license-preview/{file_type}/{index}")
def get_license_file_preview(
    file_type: str,
    index: int,
    current_user: models.user.User = Depends(get_current_user),
    effective_user: models.user.User = Depends(get_effective_user),
    ...
    """
    Returns a preview image (PNG) of the first page of the indexed file
    for the given file_type (license_picture, referrals, job_photos).
    For PDF files, renders first page via PyMuPDF.
    For images, returns the raw image bytes.
    """
    contractor = _get_contractor(effective_user, db)

    file_map = {
        "license_picture": contractor.license_picture,
        "referrals": contractor.referrals,
        "job_photos": contractor.job_photos,
    }
    ...
    if not files or not isinstance(files, list) or index >= len(files):
        raise HTTPException(status_code=404, detail="File not found")

    file_entry = files[index]
    if not isinstance(file_entry, dict) or "data" not in file_entry:
        raise HTTPException(status_code=404, detail="File data not available")

    filename = file_entry.get("filename", "")
    content_type = file_entry.get("content_type", "")
    raw_bytes = base64.b64decode(file_entry["data"])
    ...
    elif ext in (".jpg", ".jpeg") or "jpeg" in content_type:
        return Response(content=raw_bytes, media_type="image/jpeg")
    elif ext == ".png" or "png" in content_type:
...
def get_contractor_trade_info(
    ...
    return {
        "user_type": contractor.user_type if contractor.user_type else [],
...
def update_contractor_trade_info(
    ...
    if data.user_type is not None:
        ...
        existing_types = set(contractor.user_type or [])
        
        # Get existing pending user types to avoid duplicates
        existing_pending = db.query(models.user.PendingUserType).filter(
        ...
        for trade in data.user_type:
            if trade in existing_types or trade in pending_values:
    ...
    base_message = "Trade information update requested. "
    if new_pending_count > 0:
        base_message += f"{new_pending_count} new trades are pending admin approval."
    else:
        base_message += "No new trades were added."
        
    if duplicates_ignored > 0:
        base_message += f" {duplicates_ignored} trades were already in your profile or pending."

    return {
        "message": base_message,
        "user_type": contractor.user_type,  # Still returns currently approved types
    }
...
def update_contractor_location_info(
    ...
    if duplicates_ignored[0] > 0:
        base_message += f". {duplicates_ignored[0]} duplicate jurisdiction requests were ignored."
