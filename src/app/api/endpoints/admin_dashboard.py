...
class GenericRejectionRequest(BaseModel):
...
@router.get(
    "/pending-jurisdictions/summary",
    dependencies=[Depends(require_admin_token)],
    summary="Get summary of pending jurisdiction requests grouped by user email",
)
def get_pending_jurisdictions_summary(
    db: Session = Depends(get_db),
):
    """
    Returns a consolidated list of users who have pending jurisdiction requests,
    along with the total count for each.
    """
    query = (
        db.query(
            models.user.User.email,
            models.user.PendingJurisdiction.user_type,
            func.count(models.user.PendingJurisdiction.id).label("pending_count"),
        )
        .join(
            models.user.PendingJurisdiction,
            models.user.PendingJurisdiction.user_id == models.user.User.id,
        )
        .filter(
            models.user.PendingJurisdiction.status == "pending",
            models.user.User.parent_user_id == None,
        )
        .group_by(models.user.User.email, models.user.PendingJurisdiction.user_type)
        .order_by(func.count(models.user.PendingJurisdiction.id).desc())
        .all()
    )

    return [
        {"email": email, "user_type": user_type, "pending_count": count}
        for email, user_type, count in query
    ]
...
def list_pending_jurisdictions(
    ...
    email: Optional[str] = Query(
        None,
        description="Filter by user email to see detailed requests for a specific user",
    ),
    status: str = Query(
        "pending",
    ...
    Can be filtered by email to see consolidated lists for one user.
    """
    query = (
    ...
    if email:
        query = query.filter(models.user.User.email == email)
        ...
        if user:
            if pj.user_type == "Contractor":
                ...
                if contractor_profile:
                    ...
                    raw = contractor_profile.state_license_number
                    if raw:
                ...
                if supplier_profile:
                    ...
                    raw = supplier_profile.state_license_number
                    if raw:
                        if isinstance(raw, list):
                            license_numbers = [str(l) for l in raw if l]
                        else:
        ...
        results.append(
            {
                ...
                "license_numbers": license_numbers,  # All license numbers from the main account's profile
...
def reject_pending_jurisdiction(
    pending_id: int,
    body: GenericRejectionRequest,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    admin: models.user.AdminUser = Depends(require_admin_or_ops),
):
    """
    Reject a pending jurisdiction with an optional note.
    """
    pj = (
        db.query(models.user.PendingJurisdiction)
        .filter(models.user.PendingJurisdiction.id == pending_id)
    ...
    if user:
        ...
        if pj.user_type == "Contractor":
            profile = db.query(models.user.Contractor).filter(models.user.Contractor.user_id == pj.user_id).first()
            if profile and profile.company_name:
                user_name = profile.company_name
        elif pj.user_type == "Supplier":
            profile = db.query(models.user.Supplier).filter(models.user.Supplier.user_id == pj.user_id).first()
            if profile and profile.company_name:
                user_name = profile.company_name
...
@router.patch(
    "/pending-user-types/{pending_id:int}/approve",
    dependencies=[Depends(require_admin_or_ops)],
    summary="Approve a pending user type (trade/category)",
)
def approve_pending_user_type(
    pending_id: int,
    db: Session = Depends(get_db),
    admin: models.user.AdminUser = Depends(require_admin_or_ops),
):
    """
    Approve a pending user type and add it to the user's profile.
    """
    put = (
        db.query(models.user.PendingUserType)
        .filter(models.user.PendingUserType.id == pending_id)
        .first()
    )

    if not put:
        raise HTTPException(status_code=404, detail="Pending user type not found")

    if put.status != "pending":
        raise HTTPException(status_code=400, detail=f"User type already {put.status}")

    # Fetch the underlying user profile
    if put.user_role == "Contractor":
        profile = (
            db.query(models.user.Contractor)
            .filter(models.user.Contractor.user_id == put.user_id)
            .first()
        )
    else:
        profile = (
            db.query(models.user.Supplier)
            .filter(models.user.Supplier.user_id == put.user_id)
            .first()
        )
    ...
    # Append to user_type array
    current = profile.user_type or []
    if put.user_type_value not in current:
        profile.user_type = current + [put.user_type_value]
        db.add(profile)

    from datetime import datetime as _dt
    put.status = "approved"
    put.reviewed_at = _dt.utcnow()
    put.reviewed_by = admin.id
    db.add(put)
    db.commit()
    db.refresh(put)
...
@router.get(
    "/pending-user-types",
    dependencies=[Depends(require_admin_token)],
    summary="List pending user type (trade/category) requests from users",
)
def list_pending_user_types(
    user_role: Optional[str] = Query(
        None,
        description="Filter by user role: 'Contractor' or 'Supplier' (default: both)",
    ),
    status: str = Query(
        "pending",
        description="Filter by status: pending/approved/rejected (default: pending)",
    ...
    """
    Admin endpoint: view all trade / category requests that require approval.
    """
    query = db.query(models.user.PendingUserType).filter(
        models.user.PendingUserType.status == status
    )

    if user_role:
        query = query.filter(models.user.PendingUserType.user_role == user_role)

    total = query.count()
    offset = (page - 1) * per_page
    rows = (
        query.order_by(models.user.PendingUserType.created_at.desc())
        .offset(offset)
        .limit(per_page)
        .all()
    )
    ...
    for put in rows:
        user = (
            db.query(models.user.User).filter(models.user.User.id == put.user_id).first()
        )
        user_email = user.email if user else None
        user_name = None
        ...
        if put.user_role == "Contractor":
            ...
            if profile:
                user_name = profile.primary_contact_name
        else:
            profile = (
                db.query(models.user.Supplier)
                .filter(models.user.Supplier.user_id == put.user_id)
        ...
        results.append(
            {
                ...
                "reviewed_at": put.reviewed_at.isoformat() if put.reviewed_at else None,
                "reviewed_by": put.reviewed_by,
            }
        )
...
@router.patch(
    "/pending-user-types/{pending_id:int}/reject",
    dependencies=[Depends(require_admin_or_ops)],
    summary="Reject a pending user type (trade/category)",
)
def reject_pending_user_type(
    pending_id: int,
    body: GenericRejectionRequest,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    ...
    """
    Reject a pending user type with an optional note.
    """
    put = (
        db.query(models.user.PendingUserType)
        .filter(models.user.PendingUserType.id == pending_id)
    ...
    put.reviewed_at = _dt.utcnow()
    put.reviewed_by = admin.id
    put.rejection_note = body.note
    db.add(put)
    db.commit()
    db.refresh(put)

    # Send rejection email
    user = db.query(models.user.User).filter(models.user.User.id == put.user_id).first()
    if user:
        user_name = user.email
        # Try to find a company name
        contractor = db.query(models.user.Contractor).filter(models.user.Contractor.user_id == put.user_id).first()
        if contractor and contractor.company_name:
            user_name = contractor.company_name
        else:
            supplier = db.query(models.user.Supplier).filter(models.user.Supplier.user_id == put.user_id).first()
        ...
        background_tasks.add_task(
            ...
            recipient_email=user.email,
            user_name=user_name,
            category_value=put.user_type_value,
            rejection_note=put.rejection_note
        )
...
def system_ingested_jobs(
    ...
    # Review Status Filters (pending only as posted is now hard-filtered)
    status_filters = []
    if status_pending:
        status_filters.append(models.user.Job.job_review_status == "pending")
...
async def invite_admin_user(
    ...
    signup_url = (
        f"{os.getenv('FRONTEND_URL', 'https://tigerleads.vercel.app')}/admin/signup"
    )
