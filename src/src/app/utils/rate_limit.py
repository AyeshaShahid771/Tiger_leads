...
class RateLimiter:
    ...
    def is_allowed(
        ...
        recent_attempts = [ts for ts in self.attempts[key] if ts > cutoff_time]
...
async def rate_limit_by_ip(
    request: Request, max_attempts: int = 5, window_seconds: int = 300
):
...
def rate_limit_by_identifier(
    ...
    """
    check_rate_limit(
        identifier, max_attempts=max_attempts, window_seconds=window_seconds
    )
