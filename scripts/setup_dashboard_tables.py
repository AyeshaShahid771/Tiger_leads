...
def constraint_exists(db, constraint_name):
    q = text("SELECT 1 FROM pg_constraint WHERE conname = :cname LIMIT 1")
...
def run():
    ...
    try:
        ...
        if not table_exists(db, "users"):
            print(
                "Table 'users' does not exist in DB. This repository expects a users table elsewhere."
            )
        else:
            print(
                "users table exists; skipping adding 'name' and 'state' columns (use supplier/contractor tables)"
            )
        ...
        if not table_exists(db, "jobs"):
            ...
            print(
                "Skipping addition of jobs.title, jobs.description, jobs.status (use existing permit_* fields)"
            )
        ...
        if not table_exists(db, "subscriptions"):
            ...
            if column_exists(db, "subscriptions", "name"):
                ...
                db.execute(
                    text("ALTER TABLE subscriptions ADD COLUMN name VARCHAR(255) NULL")
                )
            ...
            if not column_exists(db, "subscriptions", "price"):
                ...
                db.execute(
                    text(
                        "ALTER TABLE subscriptions ADD COLUMN price NUMERIC(12,2) NULL"
                    )
                )
            else:
            ...
            if column_exists(db, "subscriptions", "status"):
                print(
                    "subscriptions.status exists (but prefer subscribers.subscription_status)"
                )
            else:
                print(
                    "Skipping creation of subscriptions.status — use subscribers table instead"
                )

            if column_exists(db, "subscriptions", "start_date"):
                print(
                    "subscriptions.start_date exists (but prefer subscribers.subscription_start_date)"
                )
            else:
                print(
                    "Skipping creation of subscriptions.start_date — use subscribers table instead"
                )

            if column_exists(db, "subscriptions", "end_date"):
                print(
                    "subscriptions.end_date exists (but prefer subscribers.subscription_renew_date)"
                )
            else:
                print(
                    "Skipping creation of subscriptions.end_date — use subscribers table instead"
                )
        ...
        try:
            if column_exists(db, "jobs", "user_id") and table_exists(db, "users"):
                ...
                if not constraint_exists(db, cname):
                    ...
                    db.execute(
                        text(
                            "ALTER TABLE jobs ADD CONSTRAINT fk_jobs_user_id FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL"
                        )
                    )
                    db.commit()
                else:
        ...
        try:
            if table_exists(db, "payments") and table_exists(db, "subscriptions"):
                ...
                if not constraint_exists(db, cname):
                    ...
                    db.execute(
                        text(
                            "ALTER TABLE payments ADD CONSTRAINT fk_payments_subscription_id FOREIGN KEY (subscription_id) REFERENCES subscriptions(id) ON DELETE SET NULL"
                        )
                    )
                    db.commit()
                else:
        ...
        def try_create_index(check_table, check_column, index_sql, skip_msg):
            try:
                if table_exists(db, check_table) and column_exists(
                    db, check_table, check_column
                ):
        ...
        try_create_index(
            ...
            "Skipping idx_users_created_at: users.created_at not present",
        )
        try_create_index(
            "jobs",
            "created_at",
            "CREATE INDEX IF NOT EXISTS idx_jobs_created_at ON jobs (created_at)",
            "Skipping idx_jobs_created_at: jobs.created_at not present",
        )
        try_create_index(
            "payments",
            "payment_date",
            "CREATE INDEX IF NOT EXISTS idx_payments_payment_date ON payments (payment_date)",
            "Skipping idx_payments_payment_date: payments.payment_date not present",
        )
